...
outdir="$1"
suboutdir="${outdir}/6-secondexperiment"
assetdir="$2"
solverdir="$3"
solver="mono-rfs.exe"
...
/opt/monodevelop/bin/mono "${solver}" -x -i=simulation -f="${assetdir}/long.world" -c="${assetdir}/movlong.in" -a=phd -p=${particlecount} -g="${cfgfile}" -r="${phdfile}"
